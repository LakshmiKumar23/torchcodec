...
# Installs the rocDecode SDK
#
# rocDecode is not preinstalled by default in the CI runners. Both the build
# and the test jobs need it (we don't ship it in the wheel, unlike nvcuvid), so
# we install it from the ROCm dnf repo.

set -euo pipefail

# rocDecode decodes via VA-API, so at runtime it needs the AMD VA-API driver
# (mesa-amdgpu-va-drivers + libva-amdgpu), not just librocdecode -- without it,
...
# to *compile* against rocdecode.h/librocdecode.so, so fall back to installing just
...
dnf install -y --refresh amdrocm-decode-devel libva-amdgpu mesa-amdgpu-va-drivers \
    || install_rocdecode_build_only
