...
tensors, on CPU and GPU (CUDA and ROCm). It also supports video and audio encoding on CPU!
...
First, make sure you have an AMD GPU with VCN (Video Core Next) hardware that can decode the
format you want. Refer to AMD's GPU support documentation for more details.

1. Install FFmpeg with ROCm support (if needed).
   TorchCodec with ROCm should work with FFmpeg versions in [4, 8].
   ...
   Note: FFmpeg doesn't have native hwcontext_hip/rocm support yet, but TorchCodec uses
   rocDecode directly for hardware acceleration on AMD GPUs.

2. Install PyTorch with ROCm support following the
   [official instructions](https://pytorch.org/get-started/locally/). Make sure to select
   the ROCm build that matches your ROCm installation.

3. Build TorchCodec from source with ROCm support:
   ...
   The `ENABLE_ROCM=1` environment variable tells the build system to enable ROCm support
...
For example:
- If you build TorchCodec with ENABLE_CUDA=1 or use the CUDA-enabled
  release of torchcodec, please review CUDA's license here:
  [Nvidia licenses](https://docs.nvidia.com/cuda/eula/index.html).
- If you build TorchCodec with ENABLE_ROCM=1 for AMD GPU support,
  please review ROCm's license here:
